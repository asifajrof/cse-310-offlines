...
string newTemp()
{
    string str = "[BP - " + to_string(symbolTable.getCurrentScope()->pushCount + symbolTable.getCurrentScope()->pushCount);
	symbolTable.getCurrentScope()->pushCount++;
	asmCodeSection = asmCodeSection + "\t" + "PUSH 0" + "\t;temp_var_decl" + "\n";
    return str;
}
...
int prevScopePushCount;	//for pop in func def
string returnLabel;
					...
					*$$->getParameterList() = *$2->getParameterList();
					...
					if(noError){
						...
						if($2->getName() == "main"){
							//main function
							mainFound = true;
							asmCodeSection = asmCodeSection + "\t" + "MOV AX, @DATA" + "\n"
											+ "\t" + "MOV DS, AX" + "\n\n";
						}
						asmCodeSection = asmCodeSection + "\t" + "PUSH BP" + "\n"
										+ "\t" + "MOV BP, SP" + "\n"
										+ $8->asmCode + "\n"
										+ returnLabel + ":" + "\n"
										+ 
										+ "\t" + "POP BP" + "\n";
						if($2->getName() == "main"){
							asmCodeSection = asmCodeSection + "\t" + "MOV AH, 4CH" + "\n"
										   + "\t" + "INT 21H" + "\n";
						}
						asmCodeSection = asmCodeSection + "\t" + "RET " + to_string($2->getParameterList()->size() + $2->getParameterList()->size()) + "\n"
									   + $2->getName() + " ENDP" + "\n";
						}
						...
						for(int i=0; i<$2->getParameterList()->size(); i++){
							...
							if(!inserted){
								...
								///other scopes
									...
									$2->getParameterList()->at(i)->setAsmName("[BP - " + to_string(symbolTable.getCurrentScope()->pushCount + symbolTable.getCurrentScope()->pushCount));
									symbolTable.getCurrentScope()->pushCount++;
									
									asmCodeSection = asmCodeSection + "\t" + "PUSH 0" + "\t;var_decl : " + $2->getParameterList()->at(i)->getName() + "\n";
								}
								$$->asmCode = asmCodeSection;
								/****************/
							}
...
variable : ID	{
		 ...
		 | ID LTHIRD expression RTHIRD	{
			...
			if(noError){
				...
				asmCodeSection = $1->asmCode + $3->asmCode;
				asmCodeSection = asmCodeSection + "\n\t" + "MOV BX, " + $3->getAsmName() + "\n"
								+ "\t" + "ADD BX, BX" + "\n";	//2 byte
...
expression : logic_expression	{
		   ...
		   | variable ASSIGNOP logic_expression	{
				...
				if(noError){
					...
					asmCodeSection = $3->asmCode + $1->asmCode;
					asmCodeSection = asmCodeSection + "\n\t" + "MOV AX, " + $3->getAsmName() + "\n"
									+ "\t" + "MOV " + variable + ", AX" + "\n";
